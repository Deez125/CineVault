...
function SidebarInset({ className, ...props }: React.ComponentProps<"main">) {
  return (
    <main
      ...
      // min-w-0 is load-bearing. This is a flex child, and a flex child defaults to
      // min-width:auto — so any content wider than the viewport (a horizontally scrolling
      // poster rail, a wide table) pushes the whole page out and puts a scrollbar on the
      // window instead of scrolling inside its own container.
      className={cn(
...
function SidebarMenu({ className, ...props }: React.ComponentProps<"ul">) {
  return (
    <ul
      ...
      data-sidebar="menu"
      // gap-0.5, not gap-0. Flush rows mean a hovered item touching the selected one paints
      // two adjacent filled rectangles with no seam, which reads as one taller selection.
...
const sidebarMenuButtonVariants = cva(
  // Hover uses --sidebar-hover; selected uses --sidebar-accent. They are deliberately
  // different values (see the colour index) so a hovered row next to the selected one is
  // still distinguishable from it.
  //
  // `data-active:hover:bg-sidebar-accent` is not redundant: without it, hovering the row you
  // are already on would drop it to the weaker hover colour, so the selection appears to
